##use TP maps script for TN maps, so read in different file but otherwise keep labels
...
data<-read.csv("Data/tn_dat.csv", header=T)

#only need hu4 identifier, lagoslakeid, date, response variable from this

data.lake <- data[, c("lagoslakeid", "tn_new", "sampleyear")]
data.reg <- data[,c("hu4_zoneid", "lagoslakeid", "tn_new", "sampleyear")]


#make median response by lake, first median for each year, then median of those
lakeyrmed<- data.lake %>% group_by(lagoslakeid, sampleyear) %>% summarise(ly.med=median(tn_new))
...
hu4lakeyrmed<- data.reg %>% group_by(hu4_zoneid, lagoslakeid, sampleyear) %>% summarise(ly.med=median(tn_new))
...
get.col.bins.cv <- function(slopes, alpha=255) {
  ...
  ii <- cut(z, breaks = c(0,.25,.5,.75,1,1.25,1.5,Inf), 
            include.lowest = TRUE)
...
png("Maps/Old/tn_meanlegend.png", width=1.5, height=.5, units='in', res=300)
par(mar = c(0,0,0,0))
plot.new()
...
text(x = seq(from = .1, to = .9, by = (.8/6)), y = rep(.2,7), labels=c("250", "500", "750", "1000", "1250", "1500", ">1500"), cex = .4)
text(x=.5, y = .85, labels = "Median TN", cex = .5)
dev.off()

png("Maps/Old/tn_cvlegend.png", width=1.5, height=.5, units = 'in', res=300)
par(mar = c(0,0,0,0))
plot.new()
points(x = seq(from = 0.1, to = 0.9, by = (.8/6)), y= rep(.5,7), pch = 22, cex = 2.75, bg = colors, col="grey30")
text(x = seq(from = .1, to = .9, by = (.8/6)), y = rep(.2,7), labels=c("0.25", "0.5", "0.75", "1.0", "1.25", "1.5", ">1.5"), cex = .5)
text(x=.5, y = .85, labels = "CV of TN", cex = .5)
dev.off()
...
pdf("Maps/Old/tn_maps.pdf",width = 8,height = 5)
