##use TP maps script for Chla maps, so read in different file but otherwise keep labels
...
data<-read.csv("Data/chl_dat.csv", header=T)

#only need hu4 identifier, lagoslakeid, date, response variable from this

data.lake <- data[, c("lagoslakeid", "chla", "sampleyear")]
data.reg <- data[,c("hu4_zoneid", "lagoslakeid", "chla", "sampleyear")]


#make median response by lake, first median for each year, then median of those
lakeyrmed<- data.lake %>% group_by(lagoslakeid, sampleyear) %>% summarise(ly.med=median(chla))
...
hu4lakeyrmed<- data.reg %>% group_by(hu4_zoneid, lagoslakeid, sampleyear) %>% summarise(ly.med=median(chla))
...
#  quants<-quantile(z, probs = c(.1, .2, .3, .4, .5, .6, .7, .8, .9), na.rm=T)
  ...
  ii <- cut(z, breaks = c(0,4, 8,12, 16, 20, 24,Inf), 
            include.lowest = TRUE)
...
get.col.bins.cv <- function(slopes, alpha=255) {
  ...
  ii <- cut(z, breaks = c(0,.3,.6,.9,1.2,1.5,1.8,Inf), 
            include.lowest = TRUE)
...
png("Maps/Old/chla_meanlegend.png", width=1.5, height=.5, units='in', res=300)
par(mar = c(0,0,0,0))
plot.new()
...
text(x = seq(from = .1, to = .9, by = (.8/6)), y = rep(.2,7), labels=c("4", "8", "12", "16", "20", "24", ">24"), cex = .5)
text(x=.5, y = .85, labels = "Median Chlorophyll a", cex = .5)
dev.off()

png("Maps/Old/chla_cvlegend.png", width=1.5, height=.5, units = 'in', res=300)
par(mar = c(0,0,0,0))
plot.new()
points(x = seq(from = 0.1, to = 0.9, by = (.8/6)), y= rep(.5,7), pch = 22, cex = 2.75, bg = colors, col="grey30")
text(x = seq(from = .1, to = .9, by = (.8/6)), y = rep(.2,7), labels=c("0.3", "0.6", "0.9", "1.2", "1.5", "1.8", ">1.8"), cex = .5)
text(x=.5, y = .85, labels = "CV of Chlorophyll a", cex = .5)
dev.off()
...
pdf("Maps/Old/chla_maps.pdf",width = 8,height = 5)
